...
public class SwordHeroAttackController : HeroAttackController
{
    ...
    public override void EnterAttackState()
    {
        ...
        else
        {
            ...
            swordHeroController.Animator.SetTrigger("Attack");
            StartCoroutine(CRWaitAndApplyDamage());
            StartCoroutine(CRWaitAndReturnIdle(attackClip));
            SoundManager.Instance.PlaySound(SoundManager.Instance.swordHeroAttack);
        }
    }
    ...
    {
        while (IsEnterState && !swordHeroController.TargetAttack.IsDead)
        {
            Vector3 targetPos = swordHeroController.TargetAttack.transform.position;
            ...
            foreach (Transform point in collisionPoints)
            {
                ...
                foreach (Collider hit in hits)
                {
                    ...
                    {
                        onTakeDameTarget.OnTakeDamage(swordHeroController.Damage);
    ...
    private IEnumerator CRWaitAndReturnIdle(AnimationClip clip)
    {
        ...
        swordHeroController.OnNextState(HeroState.Idle_State);
    }
}
