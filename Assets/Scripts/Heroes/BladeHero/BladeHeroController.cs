...
public class BladeHeroController : HeroController
{
    ...
    public override void OnInit(int level)
    {
        ...
        {
            PlayerBladeHeroConfigSO bladeHeroesConfigSO = Resources.Load<PlayerBladeHeroConfigSO>("HeroConfig/BladeHeroConfig/Player/" + heroName);
            PlayerBladeHeroData playerHeroData = bladeHeroesConfigSO.GetHeroData(level);
        ...
        {
            EnemyBladeHeroConfigSO bladeHeroesConfigSO = Resources.Load<EnemyBladeHeroConfigSO>("HeroConfig/BladeHeroConfig/Enemy/" + heroName);
            EnemyBladeHeroData enemyHeroData = bladeHeroesConfigSO.GetHeroData(level);
    ...
    public override bool IsInRange(Transform targetPos)
    {
        ...
        if(check == true)
        {
            if (distance < 2.5f) { return true; } return false;
        }
        else
        {
            if (distance < 2f) { return true; } return false;
        }
    }
