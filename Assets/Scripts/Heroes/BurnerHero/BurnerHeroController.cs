...
public class BurnerHeroController : HeroController
{
    [SerializeField] private Transform fireBurnPos = null;
    [SerializeField] private ParticleSystem chargeEffect = null;
    [SerializeField] private FireBurnController fireBurnPrefab = null;


    public TargetController TargetAttack { get; private set; }
    public Transform FireBurnPos => fireBurnPos;
    ...
    private List<FireBurnController> listFireBurnController = new List<FireBurnController>();
    ...
    public override void OnInit(int level)
    {
        ...
        {
            PlayerBurnerHeroConfigSO burnerHeroesConfigSO = Resources.Load<PlayerBurnerHeroConfigSO>("HeroConfig/BurnerHeroConfig/Player/" + heroName);
            PlayerBurnerHeroData playerHeroData = burnerHeroesConfigSO.GetHeroData(level);
            CurrentHealth = playerHeroData.health;
            InitHealth = playerHeroData.health;
            SpeedRun = playerHeroData.speedRun;
            DamagePerSecond = playerHeroData.damagePerSecond;
        ...
        {
            EnemyBurnerHeroConfigSO burnerHeroesConfigSO = Resources.Load<EnemyBurnerHeroConfigSO>("HeroConfig/BurnerHeroConfig/Enemy/" + heroName);
            EnemyBurnerHeroData enemyHeroData = burnerHeroesConfigSO.GetHeroData(level);
    ...
    /// </summary>
    /// <returns></returns>
    public FireBurnController GetFireBurnController()
    {
        //Find in the list
        FireBurnController fireBurn = listFireBurnController.Where(a => !a.gameObject.activeSelf).FirstOrDefault();

        if (fireBurn == null)
        {
            //Did not find one -> create new one
            fireBurn = Instantiate(fireBurnPrefab, Vector3.zero, Quaternion.identity);
            fireBurn.gameObject.SetActive(false);
            listFireBurnController.Add(fireBurn);
        }
